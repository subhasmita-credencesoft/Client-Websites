"use client";

import Link from "next/link";
import { PageHero } from "@/components/sections/PageHero";
import { LuxuryButton } from "@/components/ui/LuxuryButton";
import { bookingEngineUrl, rooms, imageSet } from "@/lib/data";
import { formatCurrency } from "@/lib/utils";

const inclusions = [
  "Daily gourmet breakfast for registered guests",
  "Access to pool, fitness studio, and signature wellness areas",
  "High-speed Wi-Fi and in-room refreshment setup",
  "Concierge support and evening turndown service"
];

export default function TariffPage() {
  return (
    <>
      <PageHero
        image={imageSet.lobby}
        eyebrow="Tariff"
        title="Room rates and stay inclusions"
        description="Explore current room tariffs, stay categories, and the included services available with each reservation."
        ctaHref={bookingEngineUrl}
        ctaLabel="Book Your Stay"
        secondaryHref="/rooms"
        secondaryLabel="View Rooms"
        priority
      />

      <section className="section-space">
        <div className="container-shell">
          <div className="mb-12 max-w-4xl">
            <p className="eyebrow">Rate Overview</p>
            <h2 className="display-title text-5xl">Tariffs tailored to room type, scale, and point of view.</h2>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-ivory/68">
              Rates below are indicative starting prices per night. Final pricing may vary by season, stay length,
              package selection, and celebration add-ons.
            </p>
          </div>

          <div className="overflow-hidden rounded-[32px] border border-gold/16 bg-dark-2">
            <div className="grid grid-cols-[1.4fr_0.9fr_0.8fr_0.9fr] border-b border-gold/12 px-6 py-5 text-xs uppercase tracking-[0.3em] text-gold-light">
              <div>Room Category</div>
              <div>Size</div>
              <div>Guests</div>
              <div>From / Night</div>
            </div>
            {rooms.map((room) => (
              <div
                key={room.slug}
                className="grid grid-cols-[1.4fr_0.9fr_0.8fr_0.9fr] items-center gap-4 border-b border-gold/10 px-6 py-6 last:border-b-0"
              >
                <div>
                  <h3 className="font-display text-3xl">{room.name}</h3>
                  <p className="mt-2 text-xs uppercase tracking-[0.28em] text-ivory/50">
                    {room.type} Â· {room.beds} Â· {room.view}
                  </p>
                </div>
                <div className="text-sm text-ivory/68">{room.size} sqft</div>
                <div className="text-sm text-ivory/68">{room.guests} guests</div>
                <div className="font-mono text-lg text-gold-light">{formatCurrency(room.price)}</div>
              </div>
            ))}
          </div>
        </div>
      </section>

      <section className="section-space bg-dark-2">
        <div className="container-shell grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="eyebrow">What&apos;s Included</p>
            <h2 className="display-title text-5xl">Every tariff is layered with signature Mountain Studio service.</h2>
            <div className="mt-8 space-y-4 text-lg leading-8 text-ivory/68">
              {inclusions.map((item) => (
                <p key={item}>{item}</p>
              ))}
            </div>
          </div>
          <div className="rounded-[32px] border border-gold/16 bg-dark p-8">
            <div className="mb-6 font-display text-4xl">Booking Notes</div>
            <div className="space-y-4 text-sm leading-8 text-ivory/64">
              <p>Taxes and service charges are calculated at checkout.</p>
              <p>Flexible and prepaid plans may carry different cancellation windows.</p>
              <p>Celebration setups, transfers, and private experiences can be added on request.</p>
              <p>For extended stays or group reservations, our reservations team can prepare a custom rate proposal.</p>
            </div>
            <div className="mt-8 flex flex-wrap gap-4">
              <LuxuryButton href="/contact" label="Request Custom Quote" />
              <LuxuryButton href={bookingEngineUrl} label="Reserve Now" variant="ghost" />
            </div>
          </div>
        </div>
      </section>

      <section className="section-space">
        <div className="container-shell grid gap-8 lg:grid-cols-3">
          {["Deluxe & Junior Suites", "Signature Suites & Villas", "Penthouse Stays"].map((title, index) => (
            <article key={title} className="rounded-[28px] border border-gold/16 bg-dark-2 p-8">
              <div className="font-mono text-sm tracking-[0.3em] text-gold-light">0{index + 1}</div>
              <h3 className="mt-4 font-display text-4xl">{title}</h3>
              <p className="mt-4 text-sm leading-7 text-ivory/64">
                {index === 0
                  ? "Ideal for short luxury stays, stylish weekends, and elevated city-escape bookings."
                  : index === 1
                    ? "Designed for longer, more immersive stays with larger living areas and deeper service layers."
                    : "Our most exclusive tariff tier, built for landmark celebrations, private hosting, and high-touch arrivals."}
              </p>
              <Link href="/rooms" className="mt-6 inline-block text-xs uppercase tracking-[0.28em] text-gold">
                Explore Category
              </Link>
            </article>
          ))}
        </div>
      </section>
    </>
  );
}

