...
import { CONTACT_BENEFITS, type ContactBenefit } from "../../data/sections/contactWhyUs";

function renderBenefitIcon(icon: ContactBenefit["icon"]) {
  switch (icon) {
    case "amenities":
      return (
      ...
      <Container>
        <div className="overflow-hidden rounded-3xl bg-[#f6f3ed] p-8 md:p-12">
          ...
          <div className="mt-10 grid gap-0 text-center md:grid-cols-3">
            {CONTACT_BENEFITS.map((item, index) => (
              <div
                key={item.title}
                className={`px-6 py-8 ${
