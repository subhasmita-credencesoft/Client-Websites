...
import type { PropertyDetails } from "@/data/property-details";
import { propertySourceBySlug } from "@/lib/properties/sources";
import { buildBookingEngineUrl } from "@/lib/utils/booking-engine";
import { buildHotelMateCheckAvailabilityUrl, parseHotelMatePropertyIdFromUrl } from "@/lib/api/hotelmate/urls";
import type { HotelMateAvailabilityRoom, HotelMateRateAvailability, HotelMateRoomRatePlan } from "@/lib/api/hotelmate/types";
...
export function PropertyDetailsPage({ property }: PropertyDetailsPageProps) {
  ...
  const [apiRooms, setApiRooms] = useState<HotelMateAvailabilityRoom[] | null>(null);
  ...
  useEffect(() => {
    ...
    const initialGuests = getInitialGuestCount(searchGuests);
    ...
    fetch(availabilityUrl, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (data && typeof data === "object") {
          ...
          const roomsList = Array.isArray(record.roomList) ? (record.roomList as HotelMateAvailabilityRoom[]) : [];
          setApiRooms(roomsList);
          
          const prices = roomsList
            .map((room) => getRoomPriceFromApi(room, 0))
...
function getInitialGuestCount(guestQuery: string | null | undefined) {
...
function getPrimaryRateAvailability(room: unknown): HotelMateRateAvailability | null {
  if (!room || typeof room !== "object") return null;
  const rec = room as Record<string, unknown>;
  const rates = Array.isArray(rec.ratesAndAvailabilityDtos) ? (rec.ratesAndAvailabilityDtos as HotelMateRateAvailability[]) : [];
  const openRate = rates.find((rate) => rate && typeof rate === "object" && (rate as Record<string, unknown>).status !== "Closed");
...
function getPrimaryRoomRatePlanAmount(room: unknown): number | null {
  ...
  const ratePlans = Array.isArray(rateAvailability.roomRatePlans) ? (rateAvailability.roomRatePlans as HotelMateRoomRatePlan[]) : [];
  const planWithAmount = ratePlans.find((plan) => getPositiveNumber(plan.amount) !== null);
  return planWithAmount ? getPositiveNumber(planWithAmount.amount) : null;
}
...
function findApiRoomForCuratedRoom(
  ...
  if (curatedNorm) {
    const matchByNormName = apiRooms.find((r) => normalize(r?.name) === curatedNorm);
