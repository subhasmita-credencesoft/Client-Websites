...
import { ChevronDown, Search, Utensils, AlertCircle, Loader2, ShoppingCart, Plus, Minus, X, ClipboardList, Clock, CheckCircle2, User, Mail, Phone, ShieldAlert, DollarSign } from "lucide-react";
import { propertySources, type PropertySource } from "@/lib/properties/sources";
import { formatCurrency } from "@/lib/utils/currency";
import { processRestaurantPayment, fetchCheckedInGuests, fetchDeliveryOptions } from "@/lib/api/bookone/client";
import type { CheckedInGuest, DeliveryOption, RestaurantPaymentResult } from "@/lib/api/bookone/types";
...
export function RestaurantPage() {
  ...
  const [deliveryOptions, setDeliveryOptions] = useState<(DeliveryOption | string)[]>([]);
  ...
  const [checkoutSuccess, setCheckoutSuccess] = useState<RestaurantPaymentResult | null>(null);
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      const cartItems = Object.values(cart).map((item) => ({
        code: item.product.code || item.product.productCode || item.id || "",
      ...
      setSubmitError(err instanceof Error ? err.message : "Failed to process payment. Please try again.");
    } finally {
                        ...
                        >
                          ...
                          {deliveryOptions && deliveryOptions.length > 0 ? (
                            deliveryOptions.map((opt, idx) => {
                              const optVal = typeof opt === "string" ? opt : opt.name || opt.counterName || opt.label || opt.value || opt.locationName || "";
                              const optLabel = typeof opt === "string" ? opt : opt.name || opt.counterName || opt.label || opt.description || optVal;
                              return (
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-md overflow-hidden rounded-3xl bg-white p-6 shadow-2xl animate-in zoom-in-95 duration-200 text-center border border-border/20">
            ...
            <div className="rounded-2xl border border-border/40 bg-muted/10 p-4 mb-6 text-left space-y-2.5">
              ...
              {checkoutSuccess.referenceNumber && checkoutSuccess.referenceNumber !== String(checkoutSuccess.id) && (
                <div className="flex justify-between text-xs">
