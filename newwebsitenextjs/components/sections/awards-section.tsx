...
import { awardCards } from "@/lib/data/mountain-content";
  ...
  return (
    <section data-section-id="awards" className="bg-black px-5 py-20 md:px-10">
      <div className="text-center" data-reveal>
        ...
        <p className="mx-auto mt-4 max-w-4xl text-lg text-white md:text-2xl">{content.subtitle}</p>
      </div>
      ...
      <div className="mx-auto mt-14 grid max-w-[92rem] gap-9 md:grid-cols-3">
        {awardCards.map((award) => (
          <article key={award.title} data-card className="border-x border-white/20 px-7 text-center">
            ...
            <p className="mt-8 text-2xl leading-tight text-white md:text-3xl">{award.title}</p>
            <p className="mt-3 text-xl leading-snug text-white/85 md:text-2xl">{award.subtitle}</p>
          </article>
        ))}
      </div>
